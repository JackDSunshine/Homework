                        ...
                        && (ke.isControlDown()))
                {
                    ...
                    buttonFind.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            if (start == -1)// 没有找到
                            {
                                JOptionPane.showMessageDialog(null, "“"+findText+"”"+"已经查找完毕", "TNS", JOptionPane.WARNING_MESSAGE);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            if (e.getActionCommand() == "新建") {
                ...
                JOptionPane.showMessageDialog(null,"丁惠洋设计编写","软件信息",JOptionPane. INFORMATION_MESSAGE);
...
//						myarea.replaceRange(null,0,textContent.length());
    ...
    private void copy() {
        if (myarea.getSelectedText() == null) {
            JOptionPane.showMessageDialog(null, "未选中文字！", "TNS", JOptionPane.WARNING_MESSAGE);
        }
        Clipboard clipBoard = Toolkit.getDefaultToolkit().getSystemClipboard();
        StringSelection stringSelection = new StringSelection(myarea.getSelectedText());
        if (checkTextWritten(myarea.getSelectedText())){
            clipBoard.setContents(stringSelection, null);
        }

    }

    private boolean checkTextWritten(String selectedText) {
        String str = selectedText;
        String[] strSplited = str.split("\n");
        for (int i = 0; i < strSplited.length; i++) {
            String temp = strSplited[i].trim();
            if (temp.startsWith("【")&&temp.endsWith("】")){
                //System.out.println("请填写"+temp.substring(temp.indexOf("【"),temp.indexOf("】")+1));
                String strTips = "请填写"+temp.substring(temp.indexOf("【"),temp.indexOf("】")+1);
                Component p = new Component() {};
                JOptionPane.showConfirmDialog(p,strTips,"信息填写提示",0,2);
                return false;
            }
        }
        return true;
